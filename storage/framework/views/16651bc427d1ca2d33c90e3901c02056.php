...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layout','data' => ['title' => 'سلة التسوق']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'سلة التسوق']); ?>
...
<div class="max-w-5xl mx-auto">
  ...
  <div class="text-sm breadcrumbs mb-4 text-base-content/50">
    <ul>
      <li><a href="<?php echo e(route('welcome')); ?>">الرئيسية</a></li>
      <li><a href="<?php echo e(route('products.index')); ?>">المتجر</a></li>
      <li>سلة التسوق</li>
    </ul>
  </div>
  ...
  <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
    <div>
      ...
      <p class="text-base-content/50 text-sm mt-1">
        <?php echo e(count($cart)); ?> <?php echo e(count($cart) === 1 ? 'منتج' : 'منتجات'); ?> · الشحن محسوب تلقائياً
      </p>
    </div>
    <?php if(count($cart) > 0): ?>
  ...
  <div class="grid co-main-grid gap-4 items-start">
    ...
    <div class="flex flex-col gap-3">
      ...
      <?php $lineTotal = $details['price'] * $details['quantity']; ?>
        ...
        <div class="card-body p-4 flex-row items-center gap-4">
          ...
          <div class="w-16 h-16 rounded-xl bg-base-200 border border-base-300 flex items-center justify-center flex-shrink-0 overflow-hidden relative">
            <?php if(!empty($details['image'])): ?>
              <img src="<?php echo e($details['image']); ?>" alt="<?php echo e($details['name']); ?>" class="w-full h-full object-cover"
          ...
          <div class="flex-1 min-w-0">
            <p class="font-semibold text-sm"><?php echo e($details['name']); ?></p>

            <?php if(!empty($details['brand']) || !empty($details['variant'])): ?>
            <div class="flex gap-1 mt-1 flex-wrap">
              <?php if(!empty($details['brand'])): ?>
                <span class="badge badge-sm bg-base-200 text-base-content/50 border-0"><?php echo e($details['brand']); ?></span>
              <?php endif; ?>
              <?php if(!empty($details['variant'])): ?>
                <span class="badge badge-sm bg-base-200 text-base-content/50 border-0"><?php echo e($details['variant']); ?></span>
              <?php endif; ?>
            </div>
            <?php endif; ?>

            <p class="text-xs mt-1 font-semibold" style="color:var(--accent)">
              <?php echo e(number_format($details['price'], 2)); ?> ج.م / قطعة
            </p>
            ...
            <div class="flex items-center gap-3 mt-2">
              ...
              <form action="<?php echo e(route('wishlist.saveForLater', $id)); ?>" method="POST" style="display:inline;">
                <?php echo csrf_field(); ?>
            ...
            <div class="join border border-base-300 rounded-lg overflow-hidden">
              <form action="<?php echo e(route('cart.update', $id)); ?>" method="POST">
                ...
                <button type="submit" class="join-item btn btn-xs btn-ghost w-7 h-7 min-h-0 text-base
                  <?php echo e($details['quantity'] <= 1 ? 'opacity-25 pointer-events-none' : ''); ?>">−</button>
              </form>
      ...
      <div class="alert border" style="background:rgba(201,169,110,0.05);border-color:rgba(201,169,110,0.2);">
        ...
        <div class="flex-1">
          <p class="text-xs" style="color:var(--accent)">
            أضف <strong><?php echo e(number_format($remaining, 0)); ?> ج.م</strong> فقط وستحصل على شحن مجاني
          </p>
          <progress class="progress w-full mt-1 h-1" value="<?php echo e($progress); ?>" max="100"
      ...
      <?php if(isset($suggested) && $suggested->count() > 0): ?>
      <div class="mt-2">
        <p class="text-xs font-bold tracking-widest text-base-content/40 uppercase mb-3">قد يعجبك أيضاً</p>
        <div class="grid grid-cols-2 gap-2">
          <?php $__currentLoopData = $suggested->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prod): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php
            ...
            <div class="card-body p-3 flex-row items-center gap-2">
              <div class="w-10 h-10 rounded-lg bg-base-200 border border-base-300 flex items-center justify-center flex-shrink-0 overflow-hidden">
                <?php if($prod->image_url): ?>
                  <img src="<?php echo e(asset('storage/' . $prod->image_url)); ?>" alt="<?php echo e($prod->name); ?>"
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-semibold truncate"><?php echo e(Str::limit($prod->name, 28)); ?></p>
                <p class="text-xs font-semibold mt-0.5" style="color:var(--accent)">
                  <?php echo e(number_format($sugPrice, 0)); ?> ج.م
                </p>
              </div>
              <form action="<?php echo e(route('cart.add', $prod->id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <button type="submit"
      ...
      <div class="card bg-base-100 border border-base-300 shadow-none overflow-hidden">
        ...
        <div class="p-4 border-b border-base-300">
          ...
          <p class="text-xs text-base-content/50 mt-1">
            <?php echo e(count($cart)); ?> <?php echo e(count($cart) === 1 ? 'منتج' : 'منتجات'); ?> في سلتك
          </p>
        </div>
        ...
        <div class="p-4 space-y-2">
          ...
          <div class="flex justify-between text-sm">
            ...
            <span class="font-medium <?php echo e($shipping == 0 ? 'text-success' : ''); ?>">
              <?php echo e($shipping == 0 ? 'مجاني' : number_format($shipping, 2) . ' ج.م'); ?>
          ...
          <div>
            ...
            <p class="text-3xl font-bold mt-1">
              <span class="text-xs font-normal text-base-content/40 ml-1">ج.م</span><?php echo e(number_format($total, 2)); ?>
    ...
    <div class="alert py-2 px-3 text-xs" style="background:rgba(61,158,106,0.1);border:1px solid rgba(61,158,106,0.3);color:#3d9e6a;">
        ✓ تم تطبيق كوبون <strong><?php echo e($coupon['code']); ?></strong>
        — خصم <?php echo e(number_format($discount, 2)); ?> ج.م
        <form action="<?php echo e(route('coupon.remove')); ?>" method="POST" style="display:inline;margin-right:auto;">
            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php else: ?>
    ...
    <form action="<?php echo e(route('coupon.apply')); ?>" method="POST" class="flex gap-2 mt-3">
        <?php echo csrf_field(); ?>
    ...
    <?php endif; ?>
    <?php if(session('success') && !$coupon): ?>
        <p class="text-xs mt-1" style="color:#3d9e6a;"><?php echo e(session('success')); ?></p>
    <?php endif; ?>
<?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\Users\USER\Herd\electric-store-api\resources\views/cart/index.blade.php ENDPATH**/ ?>