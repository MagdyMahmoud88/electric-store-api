...
<?php $component->withAttributes([]); ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8" dir="rtl">
        ...
        <div class="flex items-center justify-between mb-7 flex-wrap gap-3">
            ...
            <a href="<?php echo e(route('admin.brands.create')); ?>" class="btn btn-primary btn-sm gap-2">
        ...
        <div id="bulkBar"
            ...
            <form id="bulkActivateForm" method="POST" action="<?php echo e(route('admin.brands.bulk-activate')); ?>">
                <?php echo csrf_field(); ?>
            ...
            <form id="bulkHideForm" method="POST" action="<?php echo e(route('admin.brands.bulk-hide')); ?>">
                <?php echo csrf_field(); ?>
        ...
        <form method="GET" action="<?php echo e(route('admin.brands.index')); ?>"
            ...
            <label class="input input-bordered input-sm flex items-center gap-2 flex-1 min-w-48">
                ...
                <input type="text" name="search" placeholder="ابحث باسم الماركة..."
                       value="<?php echo e(request('search')); ?>" class="grow bg-transparent text-sm">
            </label>
            ...
            <select name="sort" class="select select-bordered select-sm">
                <option value="created_at" <?php if(request('sort', 'created_at') === 'created_at'): echo 'selected'; endif; ?>>الأحدث</option>
                <option value="name"          <?php if(request('sort') === 'name'): echo 'selected'; endif; ?>>الاسم أ-ي</option>
                <option value="products_count" <?php if(request('sort') === 'products_count'): echo 'selected'; endif; ?>>عدد المنتجات</option>
            </select>

            <select name="dir" class="select select-bordered select-sm">
                <option value="desc" <?php if(request('dir', 'desc') === 'desc'): echo 'selected'; endif; ?>>تنازلي</option>
                <option value="asc"  <?php if(request('dir') === 'asc'): echo 'selected'; endif; ?>>تصاعدي</option>
            </select>

            <button type="submit" class="btn btn-primary btn-sm">بحث</button>

            <?php if(request()->hasAny(['search','status','sort','dir'])): ?>
                <a href="<?php echo e(route('admin.brands.index')); ?>" class="btn btn-ghost btn-sm">مسح</a>
            <?php endif; ?>
        </form>
                ...
                <div class="card bg-base-100 border border-base-300 shadow-sm
                    ...
                    <div class="card-body items-center text-center p-4 gap-2.5">
                        ...
                        <div class="w-16 h-16 rounded-xl bg-base-200 border border-base-300
                            ...
                            <?php if($brand->logo): ?>
                                <img src="<?php echo e(asset('storage/' . $brand->logo)); ?>"
                                     alt="<?php echo e(e($brand->name)); ?>"
                                     loading="lazy"
                                     class="w-full h-full object-contain">
                            <?php else: ?>
                                <span class="text-lg font-black text-primary">
                                <?php echo e(mb_substr($brand->name, 0, 2)); ?>
                        ...
                        <p class="text-xs text-base-content/50">
                            <?php echo e($brand->products_count ?? 0); ?> منتج
                        </p>

                        
                        <?php if($brand->discount_percentage > 0): ?>
                            ...
                            <span class="text-xs font-black text-warning">
                                خصم <?php echo e($brand->discount_percentage); ?>%
                            </span>
                                <span class="text-[10px] text-base-content/40">
                                <?php if($brand->discount_expires_at): ?>
                                        ينتهي <?php echo e($brand->discount_expires_at->format('Y/m/d')); ?>
                        ...
                        <div class="badge badge-sm gap-1 <?php echo e($brand->is_active ? 'badge-success' : 'badge-ghost'); ?> font-bold">
                        ...
                        <div class="flex gap-1.5 w-full mt-1">
                            ...
                            <form method="POST"
                                  action="<?php echo e(route('admin.brands.toggle-status', $brand)); ?>"
                                  class="flex-1 toggle-form"
                                  data-name="<?php echo e(e($brand->name)); ?>"
                                  data-action="<?php echo e($brand->is_active ? 'إخفاء' : 'تفعيل'); ?>">
                                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                <button type="submit" class="btn btn-ghost btn-xs w-full">
                                    <?php echo e($brand->is_active ? 'إخفاء' : 'تفعيل'); ?>
                        ...
                        <button type="button"
                                ...
                                onclick="openDeleteModal('<?php echo e(e($brand->name)); ?>', '<?php echo e(route('admin.brands.destroy', $brand)); ?>')">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            حذف
    ...
    <dialog id="bulkDeleteModal" class="modal">
        <div class="modal-box max-w-sm text-center" dir="rtl">
            ...
            <div class="flex gap-3 justify-center">
                ...
                <form id="bulkDeleteForm" method="POST" action="<?php echo e(route('admin.brands.bulk-destroy')); ?>">
                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php endif; ?>
<?php /**PATH C:\Users\USER\Herd\electric-store-api\resources\views/admin/brands/index.blade.php ENDPATH**/ ?>