<section class="py-16 px-5 md:px-10 bg-base-100">
    ...
    <div class="max-w-[1300px] mx-auto grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr));">
        @foreach([
            ...
            ['★★★★☆','"متجر ممتاز، التشكيلة واسعة وفيها كل اللي محتاجه."','خالد إبراهيم','صاحب ورشة'],
        ] as $i => $r)
        <div class="reveal delay-{{ $i+1 }} rounded-2xl p-6 transition-all duration-300 bg-base-200 border border-base-content/10 hover:border-amber-400/20">
            <div class="text-amber-400 text-base tracking-widest mb-4">{{ $r[0] }}</div>
            <p class="text-sm italic leading-[1.8] mb-5 text-base-content/50">{{ $r[1] }}</p>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-black text-base-content">{{ $r[2] }}</p>
                    <p class="text-[11px] text-base-content/40">{{ $r[3] }}</p>
                </div>
            </div>
        </div>
        @endforeach
