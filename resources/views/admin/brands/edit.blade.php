                    ...
                    <div class="card bg-base-200 border border-base-content/10 shadow-xl">
                        <div class="card-body p-6">
                            ...
                            <div class="form-control w-full">
                                ...
                                @error('name')
                                <p class="text-error text-xs font-bold mt-1 px-1">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                <div class="flex-grow w-full">
                                    <label class="relative flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-base-content/20 rounded-2xl cursor-pointer hover:bg-base-300 transition-all group overflow-hidden">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <span class="text-3xl group-hover:scale-125 transition-all">📸</span>
                                    ...
                                    @error('logo')
                                    <p class="text-error text-xs font-bold mt-2 text-center">{{ $message }}</p>
                                    @enderror
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card bg-base-200 border border-base-content/10 shadow-xl">
                        <div class="card-body p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {{-- نسبة الخصم --}}
                                <div class="form-control w-full">
                                    <label class="label font-bold text-xs text-base-content/70 uppercase">
                                    ...
                                    <input type="number" name="discount_percentage" min="0" max="100" step="0.01"
                                           ...
                                           value="{{ old('discount_percentage', $brand->discount_percentage ?? 0) }}"
                                ...
                                <div class="form-control w-full">
                                    ...
                                    <input type="datetime-local" name="discount_expires_at"
                                           class="input input-bordered focus:input-warning bg-base-300 w-full font-bold"
                                           value="{{ old('discount_expires_at', $brand->discount_expires_at?->format('Y-m-d\TH:i')) }}" />
                            ...
                            @if($brand->discount_percentage > 0)
                                <div class="mt-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3">
                                    <span class="text-2xl">⚡</span>
                                    <div>
                                        <p class="text-sm font-black text-amber-400">
                                            خصم {{ $brand->discount_percentage }}% مطبق على {{ $brand->products()->count() }} منتج
                                        </p>
                                        @if($brand->discount_expires_at)
                                            <p class="text-xs text-base-content/50 mt-0.5">
                                                ينتهي {{ $brand->discount_expires_at->diffForHumans() }}
                                            </p>
                                        @else
                                            <p class="text-xs text-base-content/50 mt-0.5">بدون تاريخ انتهاء</p>
                                        @endif
                                    </div>
                                </div>
                            @endif
