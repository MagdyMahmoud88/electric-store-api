...
<x-layout>
                ...
                <nav class="admin-bar__nav">
                    ...
                    @foreach($navItems as $item)
                        <a href="{{ route($item['route']) }}"
                           class="admin-nav-link {{ request()->routeIs($item['route']) ? 'admin-nav-link--active' : '' }}">
                            <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" d="{{ $item['icon'] }}"/>
                            </svg>
                            {{ $item['label'] }}
                        </a>
                    @endforeach
                    ...
                    <a href="{{ route('admin.orders.index') }}"
                       class="admin-nav-link {{ request()->routeIs('admin.orders.*') ? 'admin-nav-link--active' : '' }}">
                        ...
                        الطلبات
                        @if($pendingOrders > 0)
                            <span class="admin-nav-link__badge">{{ $pendingOrders }}</span>
                        @endif
                    </a>

                    {{-- الإرجاع --}}
                    @php $pendingReturnsCount = \App\Models\ReturnRequest::where('status','pending')->count(); @endphp
                    <a href="{{ route('admin.returns.index') }}"
                       class="admin-nav-link {{ request()->routeIs('admin.returns.*') ? 'admin-nav-link--active' : '' }}">
                        <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        @if($pendingReturnsCount > 0)
                            <span class="admin-nav-link__badge">{{ $pendingReturnsCount }}</span>
                        @endif
                    </a>

                    <a href="{{ route('admin.products.create') }}" class="admin-nav-link admin-nav-link--cta">
    ...
    <div class="page-container py-8">
        ...
        {{-- ══ Pending Alerts ══ --}}
        @if($pendingOrders > 0)
            <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" class="pending-alert">
                ...
                <span><strong>{{ $pendingOrders }} {{ $pendingOrders === 1 ? 'طلب معلق' : 'طلبات معلقة' }}</strong> تحتاج مراجعة الآن</span>
        ...
        @if($pendingReturnsCount > 0)
            <a href="{{ route('admin.returns.index', ['status' => 'pending']) }}" class="pending-alert pending-alert--returns">
                <span class="pending-alert__dot pending-alert__dot--returns"></span>
                <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z"/>
                </svg>
                <span><strong>{{ $pendingReturnsCount }} {{ $pendingReturnsCount === 1 ? 'طلب إرجاع' : 'طلبات إرجاع' }}</strong> تحتاج مراجعة</span>
                <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" class="pending-alert__arrow">
                    <path stroke-linecap="round" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
        ...
        <div class="stats-grid" style="margin-bottom:12px;">
            <div class="stat-card" style="border-color:rgba(245,158,11,.3);">
                ...
                <div>
                    <p class="stat-card__val" style="font-size:16px;">{{ number_format($revenueToday, 0) }} ج.م</p>
            ...
            <div class="stat-card" style="border-color:rgba(34,197,94,.3);">
                ...
                <div>
                    <p class="stat-card__val" style="font-size:16px;">{{ number_format($revenueThisMonth, 0) }} ج.م</p>
            ...
            <div class="stat-card" style="border-color:rgba(59,130,246,.3);">
                ...
                <div>
                    <p class="stat-card__val" style="font-size:16px;">{{ number_format($revenueTotal, 0) }} ج.م</p>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalProducts }}</p>
            ...
            <a href="{{ route('admin.orders.index') }}" class="stat-card stat-card--link">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalOrders }}</p>
                    <p class="stat-card__label">إجمالي الطلبات</p>
                </div>
            </a>

            <a href="{{ route('admin.users.index') }}" class="stat-card stat-card--link">
                <div class="stat-card__icon stat-card__icon--blue">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalUsers }}</p>
                    <p class="stat-card__label">المستخدمين</p>
                </div>
            </a>

            <a href="{{ route('admin.users.index', ['sort' => 'latest']) }}" class="stat-card stat-card--link">
                ...
                <div>
                    <p class="stat-card__val">{{ $newUsersToday }}</p>
                    <p class="stat-card__label">مستخدمين اليوم</p>
                </div>
            </a>

            <a href="{{ route('admin.brands.index') }}" class="stat-card stat-card--link">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalBrands }}</p>
                    <p class="stat-card__label">الماركات</p>
                </div>
            </a>

            <a href="{{ route('admin.categories.index') }}" class="stat-card stat-card--link">
                <div class="stat-card__icon stat-card__icon--blue">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalCategories }}</p>
                    <p class="stat-card__label">الأقسام</p>
                </div>
            </a>

            <a href="{{ route('admin.coupons.index') }}" class="stat-card stat-card--link">
                <div class="stat-card__icon stat-card__icon--teal">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div>
                    <p class="stat-card__val">{{ $totalCoupons }}</p>
                    <p class="stat-card__label">الكوبونات</p>
                </div>
            </a>

            <a href="{{ route('admin.reviews.index') }}" class="stat-card stat-card--link {{ $pendingReviews > 0 ? 'stat-card--warn' : '' }}">
                ...
                <div>
                    <p class="stat-card__val {{ $pendingReviews > 0 ? 'text-amber-500' : '' }}">{{ $pendingReviews }}</p>
                    <p class="stat-card__label">تقييمات انتظار</p>
                </div>
            </a>

            <a href="{{ route('admin.returns.index') }}" class="stat-card stat-card--link {{ $pendingReturnsCount > 0 ? 'stat-card--warn' : '' }}">
                ...
                <div>
                    <p class="stat-card__val {{ $pendingReturnsCount > 0 ? 'text-orange-400' : '' }}">{{ $pendingReturnsCount }}</p>
            ...
            <div class="stat-card {{ $lowStock > 0 ? 'stat-card--warn' : '' }}">
                ...
                <div>
                    <p class="stat-card__val {{ $lowStock > 0 ? 'text-amber-500' : '' }}">{{ $lowStock }}</p>
                    <p class="stat-card__label">مخزون منخفض</p>
                </div>
            </div>

            <div class="stat-card {{ $outOfStock > 0 ? 'stat-card--danger' : '' }}">
                ...
                <div>
                    <p class="stat-card__val {{ $outOfStock > 0 ? 'text-red-500' : '' }}">{{ $outOfStock }}</p>
                    <p class="stat-card__label">نفذ من المخزون</p>
                </div>
            </div>
        </div>

        {{-- ══ Two-Column: Recent Orders + Inventory ══ --}}
                ...
                <div class="dash-card">
                    <div class="dash-card__head">
                        <div>
                            ...
                            <p class="dash-card__sub">أحدث {{ $recentOrders->count() }} طلبات</p>
                        </div>
                    ...
                    <div class="order-list">
                        ...
                        @foreach($recentOrders as $order)
                            @php [$statusLabel, $statusKey] = $statusMap[$order->status] ?? ['—', 'default']; @endphp
                            <a href="{{ route('admin.orders.show', $order->id) }}" class="order-row">
                                <div class="order-avatar">
                                    {{ strtoupper(mb_substr($order->user?->name ?? 'U', 0, 1)) }}
                                </div>
                                <div class="order-info">
                                    <p class="order-name">{{ $order->user?->name ?? 'مستخدم محذوف' }}</p>
                                    <p class="order-meta">#{{ $order->order_number }} · {{ $order->created_at->diffForHumans() }}</p>
                                </div>
                ...
                <div class="dash-card">
                    <div class="dash-card__head">
                        <div>
                            ...
                            <p class="dash-card__sub">{{ $lowStockProducts->count() }} منتجات</p>
                        </div>
                        <a href="{{ route('admin.products.index') }}" class="dash-link">
                            إدارة المخزون
                            <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" d="M15 19l-7-7 7-7"/>
                            </svg>
                    ...
                    <div class="inventory-list">
                        @foreach($lowStockProducts as $product)
                            <div class="inventory-row">
                                <x-product-image :product="$product" class="inventory-thumb" :lazy="false" />
                                <div class="inventory-info">
                                    <p class="inventory-name">{{ $product->name }}</p>
                                    <p class="inventory-cat">{{ $product->category->name ?? '—' }}{{ $product->brand ? ' · '.$product->brand->name : '' }}</p>
                                </div>
                                <div class="inventory-right">
                                    <span class="inventory-price">{{ number_format($product->discounted_price, 0) }} ج.م</span>
                                    @if($product->stock === 0)
                                        <span class="stock-badge stock-badge--none">نفذ</span>
                                    @else
                                        <span class="stock-badge stock-badge--low">{{ $product->stock }} وحدة</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
        ...
        <div class="dash-card">
            <div class="dash-card__head">
                <div>
                    ...
                    <p class="dash-card__sub">{{ $products->total() }} منتج في المخزن</p>
                </div>
                <form method="GET" action="{{ route('admin.dashboard') }}">
                    <label class="search-field">
                        ...
                        <input type="text" name="search" placeholder="بحث عن منتج..." value="{{ request('search') }}">
                    </label>
                </form>
            </div>
                    ...
                    @foreach($products as $product)
                        <div class="p-4 flex items-start gap-3">
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}"
                                 ...
                                 onerror="this.src='{{ asset('images/error.jpg') }}'">
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                    <span class="text-sm font-black" style="color:var(--electric);">{{ number_format($product->discounted_price, 2) }} ج.م</span>
                                    <span class="stock-badge {{ $product->stock > 10 ? 'stock-badge--ok' : ($product->stock > 0 ? 'stock-badge--low' : 'stock-badge--none') }}">
                                {{ $product->stock > 0 ? $product->stock.' وحدة' : 'نفذ' }}
                            </span>
                                </div>
                            </div>
                            <div class="flex flex-col gap-2 shrink-0">
                                <a href="{{ route('admin.products.edit', $product->id) }}" class="action-btn action-btn--edit">تعديل</a>
                    ...
                    <table class="products-table">
                        ...
                        @foreach($products as $product)
                            <tr>
                                <td>
                                    <div class="prod-cell">
                                        <img src="{{ asset($product->image_url ?? $product->image) }}"
                                             alt="{{ $product->name }}" class="prod-thumb"
                                             onerror="this.src='{{ asset('images/placeholder.png') }}'">
                                        <div>
                                            <p class="prod-cell__name">{{ $product->name }}</p>
                                            <p class="prod-cell__id">#{{ $product->id }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-start;">
                                        ...
                                        @if($product->brand)
                                            <span class="tag tag--purple">{{ $product->brand->name }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <span class="price-val">{{ number_format($product->discounted_price, 2) }} ج.م</span>
                                    @if($product->effective_discount > 0)
                                        <p class="price-original">{{ number_format($product->price, 2) }} ج.م</p>
                                    @endif
                                </td>
                                <td>
                                <span class="stock-badge {{ $product->stock > 10 ? 'stock-badge--ok' : ($product->stock > 0 ? 'stock-badge--low' : 'stock-badge--none') }}">
                                ...
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('admin.products.edit', $product->id) }}" class="action-btn action-btn--edit">
                                            <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @else
                <div class="empty-state">
                    ...
                    <a href="{{ route('admin.products.create') }}" class="btn-add-product">إضافة منتج الآن</a>
                </div>
            @endif
        </div>
