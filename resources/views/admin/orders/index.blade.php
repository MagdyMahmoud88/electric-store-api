...
<div class="page-container py-10" dir="rtl" style="font-family:'Cairo',sans-serif;">
    ...
    <div class="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
            ...
            <p class="text-sm mt-1" style="color:var(--text-muted);">{{ $orders->total() }} طلب إجمالاً</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-8">
        ...
        <a href="{{ request()->fullUrlWithQuery(['status' => $key, 'page' => 1]) }}"
           ...
           style="background:var(--surface);border:1px solid {{ request('status') === $key ? 'rgba(245,158,11,.4)' : 'var(--border)' }};">
            <div class="flex items-center justify-between">
                <span class="text-xl">{{ $icon }}</span>
                <span class="badge badge-{{ $color }} badge-sm font-bold">{{ $statusCounts[$key] ?? 0 }}</span>
            </div>
            <p class="text-xs font-bold" style="color:var(--text-soft);">{{ $label }}</p>
        </a>
        @endforeach
    </div>
    ...
    <div class="rounded-2xl p-5 mb-6" style="background:var(--surface);border:1px solid var(--border);">
        <form method="GET" action="{{ route('admin.orders.index') }}" class="flex flex-wrap gap-3 items-end">
            ...
            <a href="{{ route('admin.orders.index') }}" class="btn btn-sm btn-ghost"
